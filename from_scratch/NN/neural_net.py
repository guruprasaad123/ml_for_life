  ...
  def train(self, data, all_y_trues):
    ...
    for epoch in range(epochs):
      for x, y_true in zip(data, all_y_trues):
        ...
        sum_h1 = Neuron(np.array([self.w1,self.w2]),self.b1,x).sum()
        h1 = sigmoid(sum_h1) # n_h1.feed_forward()

        sum_h2 = Neuron(np.array([self.w3,self.w4]),self.b2,x).sum()
        h2 = sigmoid(sum_h2)

        sum_o1 = Neuron(np.array([self.w5,self.w6]),self.b3,np.array([h1,h2])).sum()
        o1 = sigmoid(sum_o1)
        y_pred = o1
